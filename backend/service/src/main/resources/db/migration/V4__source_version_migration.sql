...
CREATE TABLE source_versions (
                                 ...
                                 CONSTRAINT chk_source_version_number
                                     CHECK (version_number > 0),

                                 CONSTRAINT chk_source_version_size
...
ALTER TABLE sources
    ADD CONSTRAINT fk_source_latest_version
...
CREATE INDEX idx_source_version_source
    ON source_versions(source_id);

CREATE INDEX idx_source_version_status
    ON source_versions(status);

CREATE INDEX idx_source_version_created_at
    ON source_versions(created_at DESC);

CREATE INDEX idx_source_latest_version
    ON sources(latest_version_id);
...
CREATE UNIQUE INDEX uk_source_version_source_version
...
CREATE UNIQUE INDEX uk_source_version_source_checksum
    ON source_versions(source_id, checksum);